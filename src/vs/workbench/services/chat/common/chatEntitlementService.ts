...
export namespace ChatEntitlementContextKeys {
	...
	export const Setup = {
		hidden: new RawContextKey<boolean>('chatSetupHidden', true, true), 		// UmiCode: always hidden — no Copilot in this distribution.
...
export class ChatEntitlementService extends Disposable implements IChatEntitlementService {
	...
	constructor(
		...
		if (!productService.defaultChatAgent) {
			// UmiCode: no Copilot in this distribution. Explicitly set the context key
			// so sentiment.hidden === true and the status bar icon never appears.
...
export class ChatEntitlementContext extends Disposable {
	...
	private withConfiguration(state: IChatEntitlementContextState): IChatEntitlementContextState {
		// UmiCode: Copilot has been removed from this distribution. The Chat
		// status bar entry must never appear regardless of entitlement state or
