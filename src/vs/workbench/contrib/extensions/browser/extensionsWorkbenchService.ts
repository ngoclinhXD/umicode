...
**Notice:** This extension is bundled with UmiCode. It can be disabled but not uninstalled.
## Features
${this.description}
`);
		}
	...
	async getChangelog(token: CancellationToken): Promise<string> {
		...
		if (this.type === ExtensionType.System) {
			return Promise.resolve(`Please check the [UmiCode Release Notes](command:${ShowCurrentReleaseNotesActionId}) for changes to the built-in extensions.`);
		}
